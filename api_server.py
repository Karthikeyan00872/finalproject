...
# --- Email Configuration ---
# Read SMTP credentials from environment variables (set these in your .env file)
SENDER_EMAIL = os.getenv("SENDER_EMAIL")
APP_PASSWORD = os.getenv("APP_PASSWORD")

if not SENDER_EMAIL or not APP_PASSWORD:
    print("⚠️ Warning: SENDER_EMAIL or APP_PASSWORD not found in environment. Email sending will be disabled or fail until you set them in your .env file.")
...
def send_email(to_email, subject, body):
    """Send an email using Gmail SMTP (SSL). Returns True on success, False otherwise."""
    if not SENDER_EMAIL or not APP_PASSWORD:
        print("Email not configured (SENDER_EMAIL or APP_PASSWORD missing). Skipping send.")
        return False
    if not validate_email(to_email):
        print(f"Invalid recipient email: {to_email}. Skipping send.")
        return False
    try:
        msg = EmailMessage()
        msg['Subject'] = subject
...
def register():
    ...
    try:
        ...
        if user_type == 'student':
            ...
            # Send acknowledgement email to tutor (non-blocking)
            try:
                if validate_email(email):
                    subject = "Tutor Application Received"
                    body = (
                        f"Hi {full_name},\n\n"
                        "Thank you for applying to become a tutor. Your application is received and is under review by our admin team. "
                        "We will notify you via email once a decision is made.\n\nRegards,\nAdmin Team"
                    )
                    sent = send_email(email, subject, body)
                    if not sent:
                        print(f"Notice: could not send application receipt email to {email}")
                else:
                    print(f"Skipping acknowledgement email due to invalid email: {email}")
            except Exception as e:
                print(f"Error while sending acknowledgement email: {e}")
...
def approve_tutor():
    ...
    try:
        ...
        # Send approval email to tutor (best-effort)
        try:
            recipient = pending_tutor.get('email')
            full_name = pending_tutor.get('full_name', username)
            if recipient and validate_email(recipient):
                subject = "Tutor Application Approved"
                body = (
                    f"Hi {full_name},\n\n"
                    "Good news — your tutor application has been approved! You can now log in and start creating courses.\n\n"
                    "Regards,\nAdmin Team"
                )
                sent = send_email(recipient, subject, body)
                if not sent:
                    print(f"Notice: approval email could not be sent to {recipient}")
            else:
                print(f"Approval email skipped: invalid or missing recipient for {username}")
        except Exception as e:
...
def reject_tutor():
    ...
    try:
        ...
        # Send rejection email to tutor (best-effort)
        try:
            recipient = pending_tutor.get('email')
            full_name = pending_tutor.get('full_name', username)
            if recipient and validate_email(recipient):
                subject = "Tutor Application Update - Rejected"
                body = (
                    f"Hi {full_name},\n\n"
                    f"We regret to inform you that your tutor application has been rejected. Reason: {rejection_reason}\n\n"
                    "If you believe this was a mistake, please contact support.\n\nRegards,\nAdmin Team"
                )
                sent = send_email(recipient, subject, body)
                if not sent:
                    print(f"Notice: rejection email could not be sent to {recipient}")
            else:
                print(f"Rejection email skipped: invalid or missing recipient for {username}")
        except Exception as e:
