...
function downloadAllQuestions(grade) {
    if (!window.currentUsername) {
        showNotification("Please log in to download question banks.", "error");
        if (window.showLogin) showLogin('student');
        return;
    }
        ...
        .then(result => {
            if (result.success) {
                ...
                if (gradeQuestions.length === 0) {
                    showNotification(`No questions found for ${grade} grade.`, "warning");
                    return;
                }
                ...
                showNotification("Failed to load questions for download.", "error");
            }
        })
        .catch(error => {
            console.error("Download error:", error);
            showNotification("Failed to download question bank.", "error");
        });
}
